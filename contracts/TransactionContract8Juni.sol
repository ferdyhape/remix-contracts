...
contract TransactionContract {
    ...
    function getAllTransactions() public view returns (Transaction[] memory) {
        Transaction[] memory allTransactions = new Transaction[](transactionCodes.length);
    ...
    function updateStatus(string memory _transactionCode, string memory _newStatus) public {
        require(bytes(transactions[_transactionCode].transactionCode).length != 0, "Transaction does not exist");
        transactions[_transactionCode].status = _newStatus;
        emit StatusUpdated(_transactionCode, _newStatus);
    }

    function updatePaymentStatus(string memory _transactionCode, string memory _newPaymentStatus) public {
        require(bytes(transactions[_transactionCode].transactionCode).length != 0, "Transaction does not exist");
    ...
    function getTransactionByFromToUserId(uint _fromToUserId) public view returns (Transaction[] memory) {
        uint count = 0;
        for (uint i = 0; i < transactionCodes.length; i++) {
            if (transactions[transactionCodes[i]].fromToUserId == _fromToUserId) {
                count++;
            }
        }
    ...
    function getTransactionByCode(string memory _transactionCode) public view returns (Transaction memory) {
        require(bytes(transactions[_transactionCode].transactionCode).length != 0, "Transaction does not exist");
